...
#if 0 // softquake -- remove platform version
...
#else
	sprintf (ver, "(SoftQuake %s) %4.2f", SOFTQUAKE_VERSION, (float)VERSION);
	dest = conback->data + 320*186 + 320 - 11 - 8*strlen(ver);
	for (x=0 ; x<strlen(ver) ; x++)
		Draw_CharToConback (ver[x], dest+(x<<3));
#endif
...
void Draw_FadeScreenDos (void)
{
	...
	row = 4;
	rowoffset = 8 * 3;
	prow = (byte *)(vid.buffer + vid.rowbytes*row);
...
// Exactly how it was in draw.c
void Draw_FadeScreenOrig (void)
{
	int			x,y;
	byte		*pbuf;
...
void Draw_FadeScreenBlack (void)
{
	int			x,y;
	byte		*pbuf;
