...
// softquake -- Avoid buffer overflows on long strings
// Yes, I'm talking about the GL_EXTENSIONS strings!
// To do a buffered printf, I think you'd have to implement one yourself...
void Con_BufferedPrint(const char *txt)
{
	char buf[MAXPRINTMSG];
	int len = strlen(txt);
	while(len > 0)
	{
		int copy = Q_min(sizeof(buf) - 1, len);
		Q_memcpy(buf, (void *)txt, copy);
		buf[copy] = 0;
		Con_Printf("%s", buf);
		len -= copy;
		txt += copy;
