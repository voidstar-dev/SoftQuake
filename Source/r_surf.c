...
// softquake  -- Add lightmap display
// This is incomplete, but in principle it works
// The only things missing are:
// r_lightmap cvar
// Set colour palette to monochrome when lightmap is active,
// otherwise, regular colour palette
// Todo: Get rid of this if!
#if 0
static inline unsigned char R_LightmapIndex(int light, int pix)
{
#if 1
	// Todo: Set colour palette to monochrome
	if(r_lightmap.value)
	{
		// Only lightmap
		return ((unsigned char *)vid.colormap)[255 - ((light & 0xFF00) >> 8)];
	}
	else
	{
		// Regular
...
#else
// Regular color, remove the function call
// Equivalent to the old code
#define R_LightmapIndex(light, pix) ((unsigned char *)vid.colormap)[((light & 0xFF00)) + pix];
#endif
	...
	// FIXME: use delta rather than both right and left, like ASM?
		...
		for (i=0 ; i<16 ; i++)
		{
			...
			for (b=15; b>=0; b--)
			{
				...
				prowdest[b] = R_LightmapIndex(light, pix);
				light += lightstep;
			}
